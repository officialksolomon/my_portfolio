    ...
    <>
      <Col xs={12} md={change ? 6 : 8} className={`d-flex  ${change ? 'justify-content-end ' : 'justify-content-center'} align-items-center p-4`}>
        <div className='position-relative '>
          ...
          <span onMouseEnter={(event) => changeImage(event)} onMouseLeave={(event) => changeImage(event)} className=' avatar-span  position-absolute start-0 bottom-0 m-3 text-white text-center bg-black p-2 rounded opacity-75 '> Want to see Solomon? <br /> <span class="d-none d-sm-inline fw-bolder">Hover!</span> <span class=" d-sm-none fw-bolder">Click!</span></span>
        </div>
      </Col>
    </>
  )
